...
const EditReviewPage = () => {
  ...
  useEffect(() => {
    const fetchReview = async () => {
      setLoading(true);
      try {
        const reviewDoc = await getDoc(doc(db, 'reviews', booking.bookingId));
        if (reviewDoc.exists()) {
          ...
          setMedia(
            reviewData.media
              ? reviewData.media
              : (reviewData.photos || []).map((url) => ({ url, type: 'image' }))
          );
        } else {
          Alert.alert(
            t('error') || 'Error',
            t('reviewNotFound') || 'Review not found.'
          );
        }

        const servicemanDoc = await getDoc(doc(db, 'servicemen', booking.sid));
        if (servicemanDoc.exists()) {
          setServiceman(servicemanDoc.data());
        }
      } catch (error) {
        console.error('Error fetching review or serviceman data: ', error);
        Alert.alert(
          t('error') || 'Error',
          t('failedToLoadReviewData') || 'Failed to load review data.'
        );
      } finally {
        setLoading(false);
      }
    ...
    fetchReview();
  }, [booking.bookingId, booking.sid, t]);
  ...
  const handleUpdateReview = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('reviewMessage', t('reviewUpdatedSuccessfully') || 'Review Updated Successfully');
      navigation.navigate('Home');
    } catch (error) {
      console.error('Error updating review: ', error);
      Alert.alert(
        t('error') || 'Error',
        t('reviewUpdateError') || 'There was an error updating the review.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false} overScrollMode="never">
      ...
      <View style={styles.header}>
        ...
        <CustomText style={styles.headerText}>{t('editReview') || 'Edit Review'}</CustomText>
      </View>
      <View style={styles.card}>
        ...
        <CustomText style={styles.reviewLabel}>
          {t('addPhotosOrVideos') || 'Add Photos or Videos'}:
        </CustomText>
        ...
        <TouchableOpacity
          ...
          disabled={loading}
          accessibilityLabel={t('updateReview') || 'Update Review'} 
          accessibilityRole="button"
          accessibilityState={{ disabled: loading }}  state
        >
          {loading ? (
            <ActivityIndicator size="small" color={themes[theme].buttonText} />
          ) : (
